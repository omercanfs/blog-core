    ...
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-3/4 space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <input type="text" name="title" value="{{ old('title', $post->title) }}" class="w-full text-2xl font-bold text-slate-800 border-0 border-b-2 border-slate-100 focus:border-indigo-500 focus:ring-0 px-0 py-2 placeholder-slate-300 transition" required>
            </div>
        ...
        <div class="w-full md:w-1/4 space-y-6">
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 sticky top-24">
                ...
                <div class="mb-4 flex items-center justify-between bg-slate-50 p-3 rounded-lg border border-slate-100">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $post->status ? 'checked' : '' }}>
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                ...
                <div class="border-2 border-dashed border-slate-200 rounded-lg p-2 mb-3 text-center cursor-pointer hover:bg-slate-50 transition relative group" onclick="document.getElementById('imageInput').click()">
                    <img id="imagePreview" src="{{ $post->image ? asset('storage/' . $post->image) : '' }}" class="{{ $post->image ? '' : 'hidden' }} w-full h-32 object-cover rounded-md">
